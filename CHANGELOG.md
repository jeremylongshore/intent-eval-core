...
- **`skill-refiner-pass/v1` in-toto predicate body — minted staging-first by Class-1 ADR DR-082** (`lineage: dr-082@2026-06-17`; bd `bd_000-projects-0r8m.3`). The signed attestation the Skill Refiner emits when a real SkillVersion clears the `@j-rig/refiner-core` acceptance gate (significant Pareto-dominance on the behavioral dimension + non-regression on every named dimension, one-sided z-test at the stated α). **ADDITIVE** — net-new predicate URI per § 7.2 backward-compat; no breaking changes, no v1 contract changed. The footprint mirrors the v0.2.0 additive staging-predicates (retraction/v1 + dashboard-render/v1): `schemas/v1/skill-refiner-pass.schema.json` (`$id: https://evals.intentsolutions.io/skill-refiner-pass/v1.schema.json`, `additionalProperties: false`), `src/predicates/skill-refiner-pass-v1.ts` (body TS type + `SKILL_REFINER_PASS_V1_URI` + in-toto Statement wrapping), the Zod validator `src/validators/v1/skill-refiner-pass-v1.ts`, `index.json` registration with `signing_mode: ln`, the Python Pydantic mirror (`python/.../_generated/skill_refiner_pass_schema.py` + `models.py` re-export), and golden fixtures + AJV/Zod/tsd parity tests.
  ...
  - **in-toto wire discipline mirrors gate-result/v1 exactly** (DR-082 Q4) — the in-toto `subject[].digest.sha256` MUST equal `source_snapshot_hash` (sans `sha256:` prefix); each row independently verifiable; no top-level bundle signature; no bespoke envelope.
  ...
  - **Staging-only (`signing_mode: ln`)** — runs in `sigstore_staging` until ALL FOUR DR-082 Q3 production triggers hold, AND-gated (SPEC.md normative section lands · DNSSEC/CAA green on `evals.intentsolutions.io` · the authoring chamber's SEPARATE signing trust root provisioned-and-live per DR-081 · ≥1 REAL SkillVersion clears the behavioral gate on a frozen signed eval-set). No production-Rekor signing is wired.
