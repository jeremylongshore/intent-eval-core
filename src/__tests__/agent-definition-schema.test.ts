/**
 * agent-definition contract #3 — codegen-generated (DR-044 D8).
 *
 * Mirrors the contract-#1/#2 walking-skeleton tests, proving the three-artifact
 * base+overlay composition for agent-definition (the agents/*.md YAML frontmatter
 * shape):
 ...
 *   - the effective-required manifest = base ∪ overlay = the IS 8-field agent set
 *   - registered in index.json as contract #3 + exported from the validators barrel
 *
 * Upstream authority: code.claude.com/docs/en/sub-agents § "Supported frontmatter
 * fields" — "Only `name` and `description` are required" ⇒ the base floor is exactly
 * [name, description]; the IS overlay promotes three upstream-optional fields (tools,
 * model, color) and adds three net-new tracking fields (version, author, tags).
 */
...
} from '../validators/v1/authoring/agent-definition.js';
...
const FIXTURE_DIR = join(__dirname, '../../tests/authoring/v1/fixtures/agent-definition');
const BASE = 'https://github.com/jeremylongshore/intent-eval-core/schemas/authoring/v1';
const COMPOSITION_ID = `${BASE}/agent-definition.schema.json`;
const UPSTREAM_BASE_ID = `${BASE}/upstream-base/agent-definition.v1.json`;
...
beforeAll(() => {
  ...
  ajv.addSchema(loadJson(join(AUTHORING_DIR, 'upstream-base/agent-definition.v1.json')));
  ajv.addSchema(loadJson(join(AUTHORING_DIR, 'is-overlay/agent-definition.v1.json')));
  ajv.addSchema(loadJson(join(AUTHORING_DIR, 'agent-definition.schema.json')));
  validateComposition = ajv.getSchema(COMPOSITION_ID)!;
  validateBase = ajv.getSchema(UPSTREAM_BASE_ID)!;
});

describe('agent-definition — composition is pure (zero authored fields)', () => {
  it('the published schema is exactly an allOf of the three layers', () => {
    const s = loadJson(join(AUTHORING_DIR, 'agent-definition.schema.json'));
    ...
    expect(allOf[2]?.$ref).toBe(`${BASE}/is-overlay/agent-definition.v1.json`);
  });
  ...
  it('is registered in index.json as contract #3 (lifecycle SHIPPED-INTERNAL)', () => {
...
describe('agent-definition — correctness against the 40-fixture corpus', () => {
  it('accepts every valid agent (22/22 — 0 false-rejects)', () => {
...
describe('agent-definition — ajv ↔ Zod fold agreement (D8 backstop)', () => {
  it('the JSON Schema and the Zod mirror return the same verdict on all 40 fixtures', () => {
    for (const cls of ['positive', 'negative', 'edge'] as const) {
      for (const file of listJson(cls)) {
        ...
        const zodVerdict = AgentDefinitionSchema.safeParse(obj).success;
  ...
  it.each([
    ['empty tools array is accepted', { tools: [] as unknown }, true],
    ['string tools is rejected', { tools: 'Read' }, false],
    ['invalid model alias is rejected', { model: 'gpt-9' }, false],
    ['non-string model is rejected', { model: 1 }, false],
    ['fable model alias is accepted', { model: 'fable' }, true],
    ['inherit model alias is accepted', { model: 'inherit' }, true],
    ['invalid color is rejected', { color: 'rainbow' }, false],
    ['non-string color is rejected', { color: 7 }, false],
    ['cyan color is accepted', { color: 'cyan' }, true],
    ['non-string name is rejected', { name: 7 }, false],
    ['empty name is rejected', { name: '' }, false],
    ['uppercase name is rejected', { name: 'Example-Agent' }, false],
    ...
    ['string tags is rejected', { tags: 'agent' }, false],
    ...
    expect(AgentDefinitionSchema.safeParse(artifact).success).toBe(expected);
  });
  ...
  it('agrees on an over-budget description (universal disclosureMarkers fold, 1536)', () => {
    ...
    expect(AgentDefinitionSchema.safeParse(bad).success).toBe(false);
  });
});

describe('agent-definition — monotonicity (overlay strictly additive/narrowing on base)', () => {
  it('base required is exactly the upstream standardFloor [name, description]', () => {
    expect([...AGENT_DEFINITION_BASE_REQUIRED]).toEqual(['name', 'description']);
  });

  it('the JSON base + overlay required arrays are disjoint (pure addition, no re-declaration)', () => {
    const base = loadJson(join(AUTHORING_DIR, 'upstream-base/agent-definition.v1.json'));
    const overlay = loadJson(join(AUTHORING_DIR, 'is-overlay/agent-definition.v1.json'));
  ...
  it('composition required ⊇ base required (overlay only ADDS)', () => {
    for (const f of AGENT_DEFINITION_BASE_REQUIRED) {
      expect(AGENT_DEFINITION_REQUIRED_FIELDS).toContain(f);
    }
  });

  it('effective required = base ∪ overlay = the IS 8-field agent set', () => {
    expect([...AGENT_DEFINITION_REQUIRED_FIELDS].sort()).toEqual(
      ['author', 'color', 'description', 'model', 'name', 'tags', 'tools', 'version'].sort(),
    );
    expect([...AGENT_DEFINITION_OVERLAY_REQUIRED].sort()).toEqual(
  ...
  it('strict narrowing: a base-valid minimal artifact is composition-INVALID (overlay genuinely narrows)', () => {
    const minimal = { name: 'example-agent', description: 'A valid agent.' };
  ...
  it('debacle guard: dropping a required field invalidates the composition (never demoted to optional)', () => {
    ...
    for (const field of AGENT_DEFINITION_REQUIRED_FIELDS) {
